...
#!SHADER: VX2000Shader.vs
void main() {
    // Zaman ile sürekli Y ekseni dönüşü
    float angle = uTime * 0.4;
    float c = cos(angle);
    float s = sin(angle);

    // Nefes efekti
    float breathe = 1.0 + sin(uTime * 0.9) * 0.018;
    ...
    // Kenar parlaklığı
    vGlow = abs(sin(uTime * 0.6 + length(position) * 1.5)) * 0.5 + 0.5;

    gl_Position = projectionMatrix * modelViewMatrix * vec4(rotated, 1.0);
}

#!SHADER: VX2000Shader.fs
void main() {
    vec3 col = uColor * (0.5 + vGlow * 0.9);
...
void main() {
    ...
    float highlight = smoothstep(0.03, 0.0, abs(0.97-1.0 + vPos.y * 0.01));
    ...
    color += r * f * mix(0.8, 2.0, highlight) * 1.0 * 0.4;
    //color *= 1.0 + f * 1.0;
